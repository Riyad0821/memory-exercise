<template>
  <v-container class="task-type">
    <v-badge color="#050505" :content="filteredTasks.length">
    ...
    <div class="tasks-list">
      <div v-for="task in filteredTasks" :key="task.id" class="task">
        <p v-if="task.status === 'complete'" class="font-italic">
          Completed
        </p>
        <p v-else-if="task.status === 'incomplete' && !isTaskLate(task.date)" class="font-italic">
          In progress
        </p>
        <p v-else class="font-italic">
          Late
        </p>
        <h2 class="text-truncate cursor-pointer" @click="openModal(task.id)">
          {{ task.title }}
        </h2>
        <p class="cursor-pointer" @click="openModal(task.id)">
          {{ task.description }}
        </p>

        <div class="d-flex flex-row justify-end align-center">
          <v-btn v-if="task.status !== 'complete'" icon="mdi-check" color="success" size="small" title="Complete task"
            class="mr-2" @click="tasksStore.completeTask(task.id)" />

          <v-btn icon="mdi-delete" color="error" size="small" title="Delete task"
            @click="tasksStore.deleteTask(task.id)" />
        </div>
      </div>
      <task-modal :modal-active="taskModalActive" @close="toggleTaskModal">
        <div v-if="state.editable === false">
          <h1>{{ taskForModal.title }}</h1>
          <span class="font-italic" :title="formatDate(taskForModal.date)">
            {{ timeFromNow(taskForModal.date) }}
          </span>
          <p>{{ taskForModal.description }}</p>
          <v-btn v-if="taskForModal.status !== 'complete'" icon="mdi-pencil" color="black" size="small" title="Edit task"
            class="mr-2" @click="editTask" />
        </div>
        ...
        <div v-else-if="state.editable === true">
          <v-form ref="form">
            ...
            <v-text-field v-model="state.taskEditForm.title" label="Title" counter="50" max-length="50" />
            ...
            <v-date-picker v-model="state.taskEditForm.date" mode="dateTime" is24hr>
              <template #default="{ inputValue, inputEvents }">
                <v-text-field v-model="state.taskEditForm.date" :counter="max" :value="inputValue"
                  title="Date to complete" single-line placeholder="Date to complete" prepend-inner-icon="mdi-calendar"
                  readonly v-on="inputEvents" />
              </template>
            </v-date-picker>

            <v-textarea v-model="state.taskEditForm.description" label="Description" counter="1000" clearable />

            <v-btn color="primary" @click="submit(taskForModal.id)">
              Edit task
            </v-btn>
          </v-form>
        </div>
